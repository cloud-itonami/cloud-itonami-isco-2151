(ns electeng.actor
  "ElectricalEngineersActor — the ISCO-08 2151 community electeng actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one electeng operation request
  ...
  The unconditional invariant: the ElectricalEngineersAdvisor can never
  directly commit a record the ElectricalEngineersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [electeng.advisor :as advisor]
            [electeng.governor :as governor]
            [electeng.ledger :as ledger]
            [electeng.store :as store]))

(defn build-graph
  "Build a compiled ElectricalEngineersActor graph. `store` implements
  `electeng.store/Store`. `advisor` implements
  `electeng.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   ...
                   ;; means the run interrupted and a human resumed the
                   ;; thread, :commit means the governor cleared it alone.
                   ;; Before this, both wrote a byte-identical entry and the
                   ;; ledger could not show that :energize-circuit — which
                   ;; ALWAYS requires sign-off — had received it.
                   (fn [{:keys [request proposal disposition]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :circuit-id (:circuit-id proposal)
                                    :payload proposal}
